...
class VIPAuth {
    ...
    async verifyKey() {
        ...
        try {
            ...
            // Slower timing để user có thời gian đọc - 5 giây total
            for (let i = 0; i < steps.length; i++) {
                this.showStatus('loading', steps[i]);
                await new Promise(resolve => setTimeout(resolve, 1250)); // 1.25s x 4 = 5s total
            }
    ...
    validateKey(key) {
        ...
        if (window.validateVIPKey) {
            const isValid = window.validateVIPKey(key);
            console.log('✅ Using new validation function:', isValid);
            return isValid;
        }
